...
document.querySelector('button').addEventListener('click', function () {
    const textAreaValue = document.querySelector('textarea').value.split('\n');
    let counter = 1;

    for (const text of textAreaValue) {
        const [firstPart, secondPart] = text.trim().toLowerCase().split('_');
        const finalValue =
            firstPart + secondPart.at(0).toUpperCase() + secondPart.slice(1);
        console.log(`${finalValue.padEnd(25)}${'✅'.repeat(counter)}`);
